...
Game.registerMod("tasmod",{
    init:function(){
        ...
        eval('Game.updateShimmers=' + Game.updateShimmers.toString().replace("if (Game.hasBuff('Cookie storm') && Math.random()<PForPause.scaleProbabilityRate(0.5))", "if (Game.hasBuff('Cookie storm') && (Date.now*10) % 333 == 0)").replace("if (Math.random()<PForPause.scaleProbabilitySingle(Math.pow(Math.max(0,(me.time-me.minTime)/(me.maxTime-me.minTime)),5))", "if (0<PForPause.scaleProbabilitySingle(Math.pow(Math.max(0,(me.time-me.minTime)/(me.maxTime-me.minTime)),5))").replace("(Game.Has('Distilled essence of redoubled luck') && Math.random()<0.01)", "(Game.Has('Distilled essence of redoubled luck') && (Math.random()<0.01 || deorlForce == true))").replace("(Game.hasBuff('Cookie storm') && Math.random()<0.5)", "(Game.hasBuff('Cookie storm') && Math.random()<0.5 && (Math.floor(Game.T * 10) % 10)==0)"));
